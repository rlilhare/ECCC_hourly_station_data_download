    ...
    if stationNum % 6 == 0:
        ...
        for year in range(int(startYear), int(endYear)+1):
            for month in range(1, 12+1):
                response = urllib2.urlopen('http://climate.weather.gc.ca/climate_data/bulk_data_e.html?format=csv&stationID=' + stnID + '&Year=' + str(year) + '&Month=' + str(month) + '&Day=11&timeframe=1&submit=Download+Data')
                ...
                with open(outputLocation + name + '/' + name + ' ' + str(year) + ' month ' + str(month) + '.csv', "w") as text_file:
